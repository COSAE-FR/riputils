...
type Cache interface {
	...
	GetWithAge(key string) (interface{}, time.Duration, bool)
	Has(key string) bool
...
// ChangeTTL changes the cache TTL, ttl of 0 means no TTL
func (c *cacheImpl) ChangeTTL(ttl time.Duration) {
	c.Lock()
	defer c.Unlock()
	if ttl == 0 {
		c.ttl = noEvictionTTL
...
func (c *cacheImpl) Set(key string, value interface{}) {
	...
	if ent, ok := c.items[key]; ok {
		...
		ent.Value.(*cacheItem).addedAt = now
		return
	}

	// Add new item
	ent := &cacheItem{key: key, value: value, addedAt: now}
...
func (c *cacheImpl) Get(key string) (interface{}, bool) {
	value, _, found := c.GetWithAge(key)
	return value, found
}

// GetWithAge returns the key value if it's not expired, and its age
func (c *cacheImpl) GetWithAge(key string) (interface{}, time.Duration, bool) {
	c.Lock()
	defer c.Unlock()
	now := time.Now()
	if ent, ok := c.items[key]; ok {
		// Expired item check
		if now.After(ent.Value.(*cacheItem).addedAt.Add(c.ttl)) {
			c.stat.Misses++
			return nil, now.Sub(ent.Value.(*cacheItem).addedAt), false
		}
		if c.isLRU {
			c.evictList.MoveToFront(ent)
		}
		c.stat.Hits++
		return ent.Value.(*cacheItem).value, now.Sub(ent.Value.(*cacheItem).addedAt), true
	}
	c.stat.Misses++
	return nil, 0, false
}
...
func (c *cacheImpl) Peek(key string) (interface{}, bool) {
	...
	if ent, ok := c.items[key]; ok {
		// Expired item check
		if time.Now().After(ent.Value.(*cacheItem).addedAt.Add(c.ttl)) {
			c.stat.Misses++
			return nil, false
		}
		c.stat.Hits++
...
func (c *cacheImpl) DeleteExpired() {
	...
	for _, key := range c.keys() {
		if time.Now().After(c.items[key].Value.(*cacheItem).addedAt.Add(c.ttl)) {
...
func (c *cacheImpl) removeOldestIfExpired() {
	ent := c.evictList.Back()
	if ent != nil && time.Now().After(ent.Value.(*cacheItem).addedAt.Add(c.ttl)) {
		c.removeElement(ent)
	}
}
